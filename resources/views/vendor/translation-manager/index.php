...
<head>
    ...
    <title><?php echo __('Translation Manager'); ?></title>
    ...
    <div class="container-fluid">
        <div class="navbar-header">
            ...
            <a href="<?php echo action('\Barryvdh\TranslationManager\Controller@getIndex') ?>" class="navbar-brand">
                <?php echo __('Translation Manager'); ?>
            </a>
        </div>
    </div>
</header>
<div class="container-fluid">
    <p><?php echo __('Warning, translations are not visible until they are exported back to the app/lang file, using :code command or publish button.', ['code' => '<code>php artisan translation:export</code>']); ?></p>
    <div class="alert alert-success success-import" style="display:none;">
        <p><?php echo __('Done importing, processed :n items! Reload this page to refresh the groups!', ['n' => '<strong class="counter">N</strong>']); ?></p>
    </div>
    <div class="alert alert-success success-find" style="display:none;">
        <p><?php echo __('Done searching for translations, found :n items!', ['n' => '<strong class="counter">N</strong>']); ?></p>
    </div>
    <div class="alert alert-success success-publish" style="display:none;">
        <p><?php echo __('Done publishing the translations for group :group!', ['group' => $group]); ?></p>
    </div>
    <div class="alert alert-success success-publish-all" style="display:none;">
        <p><?php echo __('Done publishing the translations for all group!'); ?></p>
    </div>
                ...
                <div class="row">
                    <div class="col-sm-3">
                        <select name="replace" class="form-control">
                            <option value="0"><?php echo __('Append new translations'); ?></option>
                            <option value="1"><?php echo __('Replace existing translations'); ?></option>
                        </select>
                    </div>
                    <div class="col-sm-2">
                    <button type="submit" class="btn btn-success btn-block"  data-disable-with="Loading.."><?php echo __('Import groups'); ?></button>
                    </div>
                </div>
            </div>
        </form>
        <form class="form-find" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postFind') ?>" data-remote="true" role="form" data-confirm="<?php echo __('Are you sure you want to scan you app folder? All found translation keys will be added to the database.'); ?>">
            <div class="form-group">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="btn btn-info" data-disable-with="Searching.." ><?php echo __('Find translations in files'); ?></button>
            </div>
        </form>
        <?php endif; ?>
        <?php if(isset($group)) : ?>
            <form class="form-inline form-publish" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postPublish', $group) ?>" data-remote="true" role="form" data-confirm="<?php echo __('Are you sure you want to publish the translations group :group? This will overwrite existing language files.', ['group' => $group]); ?>">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="btn btn-info" data-disable-with="Publishing.." ><?php echo __('Publish translations'); ?></button>
                <a href="<?= action('\Barryvdh\TranslationManager\Controller@getIndex') ?>" class="btn btn-default"><?php echo __('Back'); ?></a>
            </form>
        <?php endif; ?>
    </p>
    <form role="form" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAddGroup') ?>">
        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
        <div class="form-group">
            <p><?php echo __('Choose a group to display the group translations. If no groups are visisble, make sure you have run the migrations and imported the translations.'); ?></p>
        ...
        <div class="form-group">
            <label><?php echo __('Enter a new group name and start edit translations in that group'); ?></label>
            <input type="text" class="form-control" name="new-group" />
        </div>
        <div class="form-group">
            <input type="submit" class="btn btn-default" name="add-group" value="<?php echo __('Add and edit keys'); ?>" />
        </div>
    </form>
    <?php if($group): ?>
        <form action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAdd', array($group)) ?>" method="POST"  role="form">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <div class="form-group">
                <label><?php echo __('Add new keys to this group'); ?></label>
                <textarea class="form-control" rows="3" name="keys" placeholder="<?php echo __('Add 1 key per line, without the group prefix'); ?>"></textarea>
            </div>
            <div class="form-group">
        ...
        <div class="row">
            <div class="col-sm-2">
                <span class="btn btn-default enable-auto-translate-group"><?php echo __('Use Auto Translate'); ?></span>
            </div>
        </div>
        <form class="form-add-locale autotranslate-block-group hidden" method="POST" role="form" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postTranslateMissing') ?>">
            ...
            <div class="row">
                <div class="col-sm-6">
                    <div class="form-group">
                        <label for="base-locale"><?php echo __('Base Locale for Auto Translations'); ?></label>
                    ...
                    <div class="form-group">
                        <label for="new-locale"><?php echo __('Enter target locale key'); ?></label>
                        <input type="text" name="new-locale" class="form-control" id="new-locale" placeholder="<?php echo __('Enter target locale key'); ?>" />
                    </div>
                    <?php if(!config('laravel_google_translate.google_translate_api_key')): ?>
                        <p>
                            <code><?php echo __('If you would like to use Google Translate API, install tanmuhittin/laravel-google-translate and enter your Google Translate API key to config file laravel_google_translate'); ?></code>
                        </p>
                    <?php endif; ?>
                    <div class="form-group">
                        <input type="hidden" name="with-translations" value="1">
                        <input type="hidden" name="file" value="<?= $group ?>">
                        <button type="submit" class="btn btn-default btn-block"  data-disable-with="Adding.."><?php echo __('Auto translate missing translations'); ?></button>
                    </div>
                </div>
            </div>
        </form>
        <hr>
    <h4><?php echo __('Total: '); ?><?= $numTranslations ?><?php echo __(', changed: '); ?><?= $numChanged ?></h4>
        <table class="table">
            <thead style="position: sticky; top: 0; background: #fff;">
            <tr>
                <th width="15%"><?php echo __('Key'); ?></th>
                <?php foreach ($locales as $locale): ?>
                    <th><?= $locale ?></th>
                <?php endforeach; ?>
    ...
    <?php else: ?>
        <fieldset>
            <legend><?php echo __('Supported locales'); ?></legend>
            <p>
                <?php echo __('Current supported locales:'); ?>
            </p>
            <form  class="form-remove-locale" method="POST" role="form" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postRemoveLocale') ?>" data-confirm="<?php echo __('Are you sure to remove this locale and all of data?'); ?>">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            ...
            <form class="form-add-locale" method="POST" role="form" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAddLocale') ?>">
                ...
                <div class="form-group">
                    <p>
                        <?php echo __('Enter new locale key:'); ?>
                    </p>
                    <div class="row">
                        <div class="col-sm-3">
                        ...
                        <div class="col-sm-2">
                            <button type="submit" class="btn btn-default btn-block"  data-disable-with="Adding.."><?php echo __('Add new locale'); ?></button>
                        </div>
                    </div>
                </div>
            </form>
        ...
        <fieldset>
            <legend><?php echo __('Export all translations'); ?></legend>
            <form class="form-inline form-publish-all" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postPublish', '*') ?>" data-remote="true" role="form" data-confirm="<?php echo __('Are you sure you want to publish all translations group? This will overwrite existing language files.'); ?>">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="btn btn-primary" data-disable-with="Publishing.." ><?php echo __('Publish all'); ?></button>
            </form>
        </fieldset>
